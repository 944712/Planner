...
class CommentTableViewController: UITableViewController {
    ...
    var commentArray: [NSManagedObject] = []
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        do {
            commentArray = try context.fetch(fetchRequest)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let doplan = commentArray[indexPath.row]
        
        let dbDate: Date? = doplan.value(forKey: "today") as? Date
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            context.delete(commentArray[indexPath.row])
            do {
                try context.save()
                print("deleted!")
            } catch let error as NSError {
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toTableView" {
            if let destination = segue.destination as? DetailCommentViewController {
                if let selectedIndex = self.tableView.indexPathsForSelectedRows?.first?.row {
                    destination.detailComment = commentArray[selectedIndex]
                }
            } }
    }
